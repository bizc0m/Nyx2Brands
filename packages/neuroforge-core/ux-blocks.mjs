import {compositionFor,compositionCSS,validSession} from './compositions.mjs';
...
export function mountUX(config){
 ...
 if(initial){selected=initial.selected;compare=initial.compare.filter(id=>refs.some(r=>r.id===id));favorites=initial.favorites;notes={...initial.notes};clips.push(...initial.clips);query=initial.query;category=initial.category;favOnly=initial.favOnly;}


 const escape=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 const say=s=>{$('#notice').textContent=s;};
 const session=()=>({selected:selected||'',compare,favorites,notes,clips,query,category,favOnly,list:!!$('#items')?.classList.contains('list'),position:Number($('#scrub')?.value||0)});
...
export function uxHTML(project){
 ...
 const composition=compositionFor(project);
 const config={id:project.project.id,blocks:ids,references:refs,state:validSession(project.integration?.session)?project.integration.session:null};
 ...
 ${panel('library','Bibliothèque','<div class="row"><input id="search" maxlength="200" aria-label="Rechercher les références" placeholder="Rechercher…"><select id="categories" aria-label="Catégorie"></select><button id="favorites" data-action="favorites" aria-pressed="false">Favoris</button><button data-action="list" aria-pressed="false">Vue liste</button><label>Taille <input id="density" aria-label="Taille des cartes" type="range" min="150" max="340" value="200"></label></div><p id="count" aria-live="polite"></p><div id="items"></div>')}
 ${panel('compare','Comparer les références','<div id="comparison"></div>')}
 ${panel('inspector','Inspecteur','<div id="inspector"><h3 id="inspector-title"></h3><p id="inspector-detail"></p><a id="reference-link" target="_blank" rel="noreferrer">Source ↗</a><label><p>Mes notes</p><textarea id="note" maxlength="8000" aria-label="Notes sur la référence"></textarea></label></div>')}
