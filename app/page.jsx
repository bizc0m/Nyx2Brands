...
export default function Home() {
  ...
  async function generateIntegration() {
    ...
    try {
      ...
      const exportProject=clone(project);if(!exportProject.integration.path.trim())exportProject.integration.path=`./${project.project.id}`;
      const files = integrationFiles(exportProject, catalogueSources(await response.text()),await baseResponse.text());
  ...
  return <main className={'atelier nyx-app '+(settingsOpen?'settings-open':'')}>
    ...
    {workspace === 'catalogue' ? <section className="legacy-composer"><iframe ref={catalogueRef} onLoad={syncCatalogue} src="./moteur-ux.html" title="Composeur avancé Moteur UX" /></section> : <div className={'atelier-workbench '+(step === 'design' ? 'design-layout' : step === 'export' ? 'export-layout' : '')}>
      ...
      {step === 'design' && <>
        ...
        <section className="original-design-card compact-theme-controls"><header><span>Theme actif</span><strong>{DRESSING_NAMES[integration.dressing||'nyx']}</strong></header>{integration.dressing==='glass-dashboard'&&<label>Flou · {integration.dressingSettings?.blur??16} px<input aria-label="Flou du verre" type="range" min="0" max="24" value={integration.dressingSettings?.blur??16} onChange={e=>updateIntegration('dressingSettings',{...integration.dressingSettings,blur:Number(e.target.value)})}/></label>}<div><button onClick={toggleNavigation}>Navigation</button>{integration.dressing==='glass-dashboard'&&<button onClick={()=>updateIntegration('dressingSettings',{blur:16,palette:false})}>Couleurs originales</button>}<button disabled={previousDressing===null} onClick={()=>{updateIntegration('dressing',previousDressing);setPreviousDressing(null);}}>Revenir au theme précédent</button></div>{integration.dressing==='glass-dashboard'&&<details><summary>Comparer avec l’original</summary><div><button onClick={()=>{setPreview('codepen');setSettingsOpen(false);}}>Voir la référence</button><a href="https://codepen.io/gestok/pen/YzLBVOp" target="_blank" rel="noreferrer">Source et attribution ↗</a></div></details>}</section>
      ...
      {step === 'export' && <div className="export-panel">        <section className="integration-generator" aria-label="Générateur final">
          ...
          <label className="full-field">Chemin du projet cible · facultatif<input value={integration.path} placeholder={`./${project.project.id}`} onChange={event => updateIntegration('path',event.target.value)} /></label>
          ...
          <button className="primary-export" disabled={generating || validation.errors.length > 0 || (!integration.features.length && !integration.blocks?.length)} onClick={generateIntegration}><Download />{generating ? 'Génération…' : 'Télécharger le kit d’intégration ZIP'}</button>
          <p className="integration-help">{!integration.features.length && !integration.blocks?.length ? 'Choisissez au moins une fonction ou un bloc. ' : ''}{!integration.path.trim() ? `Chemin de repli : ./${project.project.id}. ` : ''}Le ZIP contient le pack réimportable, les sources, le skin, les images, un adaptateur web et INTEGRATION.md. L’intégration des fonctions à l’app cible reste à exécuter.</p>
        </section>
        ...
        {output === 'pack' ? <>
          ...
          {validation.errors.length > 0 ? <button className="primary-export" disabled><Download />Exporter le pack commun</button> : <a className="primary-export" download={`${project.project.id}.neuroforge.json`} href={`data:application/json;charset=utf-8,${encodeURIComponent(JSON.stringify(project,null,2))}`} onClick={()=>setMessage('Pack téléchargé.')}><Download />Télécharger le pack commun</a>}
